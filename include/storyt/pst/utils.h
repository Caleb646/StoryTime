...
namespace storyt::utils {
    ...
    const std::vector<uint8_t> BTYPE_VALUES = {
        0x6C,
        0x7C,
        0x8C,
        0x9C,
    ...
    const std::vector<uint32_t> PROPERTY_TYPE_VALUES = {
        0x0002,
        0x0003,
        0x0004,
        0x0005,
    ...
    std::string NIDTypeToString(T t)
    {
        ...
        case 0x21: return "NID_TYPE_MESSAGE_STORE";
        default: return "Unknown NID Type";
        }
    }
    ...
    public:
        ...
        std::vector<types::byte_t> split(size_t size)
        {
            std::vector<types::byte_t> res{};
            res.reserve(size * 2);
            for (size_t i = 0; i < size; ++i)
            {
                const uint8_t byte = read<uint8_t>(1);
                const uint8_t leasSig = byte & 0xFU;
                const uint8_t mostSig = byte & 0xF0U;
                res.push_back(leasSig);
                res.push_back(mostSig);
            }
            return res;
        }
        ...
        EntryType entry(size_t size, Args&& ... args)
        {
            const size_t start = m_start;
            m_start += size;
            return EntryType(slice(m_bytes, start, start + size, size), std::forward<Args>(args)...);
        }

        template<typename EntryType, typename ...Args>
        std::vector<EntryType> entries(size_t nEntries, size_t singleEntrySize, Args&& ... args)
        {
        ...
        ByteView& setStart(size_t newStart)
        {
            m_start = newStart;
            return *this;
        }
    ...
    */
    ...
    namespace ms
    {
        ...
        /**
            * @brief The following algorithm is used for NDB_CRYPT_PERMUTE. 
            * Note that the data is encoded or decoded in place.
            * @param pv = the data buffer to encode/decode.
            * @param cb = the size of the data buffer
            * @param fEncrypt = specifies whether the input data is encoded (TRUE) or decoded (FALSE)
        */
        ...
        /**
            * @brief The following algorithm is used for NDB_CRYPT_CYCLIC.
            * Note that this is a symmetric cipher that is used to both encode and decode.  
            * Note that the data is encoded or decoded in place
            * @param pv = the data buffer to encode/decode
            * @param cb = the size of the data buffer
            * @param dwKey = the lower DWORD of the BID associated with this data block
        */
